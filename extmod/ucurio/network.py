...
def tcp_server_socket(host, port, family=socket.AF_INET, backlog=5,
                      reuse_address=True, reuse_port=False):

    sock = socket.socket(family, socket.SOCK_STREAM)
    host_info = socket.getaddrinfo(host, port)[0] 
    try:
        sock.setblocking(False)
        if reuse_address:
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

        print(sock._socket, host_info)
        sock.bind(host_info[-1])
        sock.listen(backlog)
    except Exception:
        print("OOPS!")
        sock._socket.close()
...
async def tcp_server(host, port, client_connected_task, *,
                     family=socket.AF_INET, backlog=5, ssl=None,
                     reuse_address=True, reuse_port=False):
