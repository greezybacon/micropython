...
def _format_stack(task, complete=False):
    ...
    for f in _get_stack(task.coro):
        ...
        name = co.co_name
        if not complete and path_dirname(filename) == dirname:
            continue
        #if filename not in checked:
        #    checked.add(filename)
        #    linecache.checkcache(filename)
        #line = linecache.getline(filename, lineno, f.f_globals)
        extracted_list.append((filename, lineno, name, ''))
    if not extracted_list:
        resp = f'No stack for {repr(task)}'
    else:
        resp = f'Stack for {repr(task)} (most recent call last):\n'
        resp += '\n'.join(repr(x) for x in extracted_list)
    return resp
...
def _where(task):
    ...
    for f in _get_stack(task.coro):
        ...
        filename = co.co_filename
        name = co.co_name
        if path_dirname(filename) == dirname:
            continue
...
class Task(object):
    ...
    def __repr__(self):
        return f'{type(self).__name__}(id={self.id}, name={repr(self.name)}, state={repr(self.state)})'

    def __str__(self):
        filename, lineno = _where(self)
        if filename:
            return f'{repr(self)} at {filename}:{lineno}'
        else:
            return repr(self)
    ...
    async def cancel(self, *, exc=TaskCancelled, blocking=True):
        ...
        if self.terminated:
            self.joined = True
            return
        await traps._cancel_task(self, exc=exc)
        if blocking:
            await self.wait()
...
class TaskGroup(object):
    ...
    def __init__(self, tasks=(), *, wait=all):
        ...
        self._finished = deque(tuple(), 5) # All finished tasks
...
async def current_task():
    ...
    '''
    return await traps._get_current()
