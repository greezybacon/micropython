...
try:
    ...
    WantRead = (OSError, SSLWantReadError)
    WantWrite = (OSError, SSLWantWriteError)
except ImportError:
    WantRead = (OSError,)
    WantWrite = (OSError,)
...
class Socket(object):
    ...
    def __repr__(self):
        return f'<curio.Socket {repr(self._socket)}>'
    ...
    def as_stream(self):
        ...
        '''
        return FileStream(self._socket)
    ...
    async def sendto(self, bytes, address):
        while True:
            try:
                return self._socket.sendto(bytes, address)
            except WantWrite:
                await _write_wait(self._socket)
            except WantRead:      # pragma: no cover
                await _read_wait(self._socket)
    ...
    async def close(self):
        if self._socket:
            await _io_release(self._socket)
            self._socket.close()
        self._socket = None
...
class StreamBase(object):
    ...
    def __repr__(self):
        return f'<curio.{type(self).__name__} {repr(self._file)}>'
    ...
    async def read(self, maxbytes=-1):
        ...
        if buf:
            if maxbytes < 0 or len(buf) <= maxbytes:
                data = bytes(buf)
                # XXX: Micropython doesn't support bytearray::clear()
                buf = self._buffer = bytearray()
            else:
    ...
    async def readline(self, maxlen=None, separator=b'\n'):
        while True:
            nl_index = self._buffer.find(separator)
            if nl_index >= 0:
...
class FileStream(StreamBase):
    ...
    async def flush(self):
        # As written for Micropython, ::write() will also flush
        pass