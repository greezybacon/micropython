...
class Kernel(object):
    ...
    def __init__(self, *, selector=None, debug=None, activations=None, taskcls=Task,
        ...
        # I/O Selector setup
        self._selector = selector if selector else PollSelector()
    ...
    def _make_kernel_runtime(kernel):
        ...
        ready = deque(tuple(), 5)               # Ready queue
        ...
        def register_event(fileobj, event, task):
            try:
                ...
                selector_modify(fileobj, mask | event,
                                (task, wtask) if event == POLLIN else (rtask, task))
                selector_getkey(fileobj)
            except KeyError:
                selector_register(fileobj, event,
                                  (task, None) if event == POLLIN else (None, task))
        ...
        def unregister_event(fileobj, event):
            ...
            if not mask:
                ...
                selector_modify(fileobj, mask,
                                (None, wtask) if event == POLLIN else (rtask, None))
        ...
        #
        #   yield ('trap_io', sock, POLLIN, 'READ_WAIT')
        #
        ...
        kernel._traps = traps = { f.__name__ : f for f in (
            trap_io, trap_io_release, trap_io_waiting, trap_future_wait,
            trap_spawn, trap_cancel_task, trap_sched_wait, trap_sched_wake,
            trap_clock, trap_sleep, trap_set_timeout, trap_unset_timeout,
            ...
            while True:
                ...
                for key, mask in events:
                    ...
                    if mask & POLLIN:
                        ...
                        # I/O event in _last_io and leave the task registered on
                        # the event loop.  If it performs the same I/O operation
                        # again, it will get a speed boost from not having to
                        # re-register its event. However, it's not safe to use
                        # this optimization with bare integer fds.  These fds
                        # often get reused and there is a possibility that a fd
                        # will get closed and reopened on a different resource
